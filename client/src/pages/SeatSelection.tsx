...
import { ArrowDownAZ, Accessibility, Ticket, ChevronUp, Minus, Plus, Trash2 } from "lucide-react";
          ...
          <div className="px-4">
            ...
            <div className="mt-6 flex gap-3 items-start text-xs text-[#008CBA] bg-blue-50 rounded-xl p-4">
               ...
               <div className="leading-relaxed">
                  ...
                  <span className="underline font-medium">accessibility.tickets@sc.qa</span>
               </div>
            </div>
          </div>
        </>
        ...
        <div className="px-4">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No tickets selected</h3>
              <p className="text-gray-500 text-sm">Select tickets from the price list to add them to your cart</p>
            </div>
          ) : (
              ...
              {items.map((item: CartItemWithDetails) => (
                <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <span className="text-sm font-bold text-gray-900">{item.categoryName || 'Category'}</span>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                    ...
                    <span className="text-lg font-bold text-[#8A1538]">QAR{(item.price || 0) * item.quantity}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg">
          ...
          >
            <div className="flex items-center gap-2 text-gray-700">
              <ChevronUp className={`w-4 h-4 transition-transform ${showCartSummary ? 'rotate-180' : ''}`} />
              <Ticket className="w-4 h-4" />
              <span className="font-semibold">{totalItems} Tickets</span>
            </div>
            <span className="text-lg font-bold text-gray-900">QAR{totalPrice}</span>
          </button>
            ...
            <div className="px-4 py-3 bg-gray-50 max-h-40 overflow-y-auto">
              {items.map((item: CartItemWithDetails) => (
                <div key={item.id} className="flex justify-between text-sm py-1.5">
                  <span className="text-gray-600">{item.quantity}x {item.categoryName}</span>
                  <span className="font-medium">QAR{(item.price || 0) * item.quantity}</span>
                </div>
              ))}
            </div>
          )}
